...
export const BadgeCanvas = forwardRef<BadgeCanvasRef, BadgeCanvasProps>(
  (
    ...
    const handleCanvasClick = (e: React.MouseEvent<HTMLDivElement>) => {
      if (!activePoint || !containerRef.current || !image || !onPointClick)
        return;

      const rect = containerRef.current.getBoundingClientRect();
      const x = Math.round(
        ((e.clientX - rect.left) / rect.width) * imageSize.width,
      );
      const y = Math.round(
        ((e.clientY - rect.top) / rect.height) * imageSize.height,
      );
    ...
    useEffect(() => {
      ...
      img.onload = () => {
        ...
        if (
          ...
          const initialNameSize = Math.max(
            24,
            Math.min(areaHeight * 0.5, areaWidth * 0.15),
          );
          const nameFontSize = findOptimalFontSize(
            badgeData.name,
            maxTextWidth,
            initialNameSize,
            14,
